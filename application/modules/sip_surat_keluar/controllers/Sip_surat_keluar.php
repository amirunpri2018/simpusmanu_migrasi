...
class Sip_surat_keluar extends CI_Controller {
    ...
    function index() {
        $data['surat'] = $this->Model_surat_keluar->kode_surat();
        $this->template->display('sip_surat_keluar_list',$data);
    }
    ...
    function view_data() {
        ...
        $getdata = $this->Model_surat_keluar->getSuratKeluar($tahun,$kode);        
        foreach ($getdata as $q) {
            ...
            $query[] = array(
                ...
                'action' => array(anchor('sip_surat_keluar/read/' . $q->id_surat, '<i class="btn btn-sm btn-info fa fa-eye" data-toggle="tooltip" title="Baca Surat"></i>') . " " .
                    anchor('sip_surat_keluar/update/' . $q->id_surat, '<i class="btn btn-sm btn-info glyphicon glyphicon-edit" data-toggle="tooltip" title="Edit"></i>') . " " .
                    anchor('sip_surat_keluar/delete/' . $q->id_surat, '<i class="btn btn-sm btn-info glyphicon glyphicon-trash" data-toggle="tooltip" title="Delete"></i>', array('onclick' => "return confirm('Data Akan di Hapus?')")))
            );
        }
    ...
    public function read($id) {
        $row = $this->Model_surat_keluar->get_by_id($id);
        if ($row) {
            $data = array(
            ...
            redirect(site_url('sip_surat_keluar'));
        }
    }
    ...
    function create() {
        $data = array(
            'button' => 'Create',
            'action' => site_url('sip_surat_keluar/create_action'),
    ...
    public function create_action() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Model_surat_keluar->insert($data);
            $this->session->set_flashdata('message', 'Create Record Success');
            redirect('sip_surat_keluar');
        }
    }
    ...
    public function update($id) {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('sip_surat_keluar/update_action'),
    ...
    public function update_action() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->Model_surat_keluar->update($this->input->post('id_surat', TRUE), $data);
