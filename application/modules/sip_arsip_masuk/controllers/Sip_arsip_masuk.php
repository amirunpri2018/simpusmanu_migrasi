...
class Sip_arsip_masuk extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Model_arsip_masuk');
        chek_session();
        cek_menu();
    }
    ...
    function index() {
        ...
        $this->template->display('sip_surat_masuk_list', $data);
    }
    ...
    function cariTglAwal() {
        ...
        $data = $this->db->query("SELECT * FROM sip_data_surat WHERE tanggal_surat='$awal' AND kategori_surat='Surat Masuk' ORDER BY id_surat DESC")->result();
        echo "<label>DATA SURAT MASUK TANGGAL $tgl_awal </label>";
        echo '<a href="' . base_url('sip_arsip_masuk/cetak/') . '" target="_blank" ><i class="btn btn-md fa fa-print" data-toggle="tooltip" title="Print"></i>';
    ...
    function cariTglAkhir() {
        ...
        $data = $this->db->query("SELECT * FROM sip_data_surat WHERE tanggal_surat BETWEEN '$awal' AND '$akhir' AND kategori_surat='Surat Masuk' ORDER BY id_surat DESC")->result();
        echo "<label>DATA SURAT MASUK TANGGAL $tgl_awal s/d TANGGAL $tgl_akhir</label>";
        echo '<a href="' . base_url('sip_arsip_masuk/cetak/') . '" target="_blank" ><i class="btn btn-md fa fa-print" data-toggle="tooltip" title="Print"></i>';
        echo "<table class='table table-bordered'>
                <tr>
                  <th>No</th>
    ...
    function cetak() {
        ...
        if (empty($awal) && empty($akhir)) {
            ...
            $header = "<label>DATA SURAT MASUK TANGGAL $tgl_awal s/d $tgl_akhir <br>POLITEKNIK PUSMANU PEKALONGAN</label>";
        }
        $data = $this->db->query("SELECT * FROM sip_data_surat WHERE tanggal_surat BETWEEN '$awal' AND '$akhir' AND kategori_surat='Surat Masuk' ORDER BY id_surat ASC")->result();
        echo '<head>
                <style>
                table {
    ...
    function excel() {
        ...
        $namaFile = "data_surat_masuk_$awal-$akhir.xls";
        $judul = "data_surat_masuk";
        $tablehead = 0;
        $tablebody = 1;
        $nourut = 1;
        //penulisan header
        ...
        $getdata = $this->db->query("SELECT * FROM sip_data_surat WHERE tanggal_surat BETWEEN '$tglawal' AND '$tglakhir' AND kategori_surat='Surat Masuk' ORDER BY id_surat ASC")->result();
        //$getdata = $this->db->get_where('sip_data_surat', array('kategori_surat' => 'Surat Masuk'))->result();
